...
export const ExploreView: React.FC<ExploreViewProps> = ({
  ...
  const handleRelatedQueryClick = useCallback(
    (query: string) => {
      if (onRelatedQueryClick) onRelatedQueryClick(query);
      handleSearch(query);
    },
  ...
  return (
    <div className={`w-full min-h-screen flex flex-col`} ref={containerRef}>
      {showInitialSearch ? (
                ...
                <div className="max-w-3xl mx-auto">
                  {message.type === "user" ? (
                    <div className="w-full">
                      <div className="flex-1 text-base sm:text-lg font-semibold text-gray-100 pt-10">
                        {message.content}
                      </div>
                    </div>
                  ) : (
